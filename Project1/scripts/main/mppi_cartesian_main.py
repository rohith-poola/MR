...
# Store the optimal position in the csv for easy visualization.
# It's Working ......!! Need to tune some parameters but went close to 9cm to the goal position
# ============================================== 
...
output_csv_path = os.path.join(output_dir, f"mppi_cartesian_samples_with_opt_{timestamp}.csv")
...
with open(output_csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    header = ["goal_id", "iteration_id", "xo", "yo", "zo"]
    for i in range(N):
...
for goal_id, goal_pos in enumerate(goal_points, start=1):
    ...
    for iteration in range(1, max_iters + 1):
        ...
        # --- Get optimal EE position ---
        for j in range(num_joints):
            p.resetJointState(robot_id, j, next_joint_angles[j])
        optimal_pos = p.getLinkState(robot_id, ee_link_index)[0]  # (xo, yo, zo)
